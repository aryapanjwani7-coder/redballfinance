...
(function () {
  ...
  async function renderCashCard() {
    try {
      const sum = await jfetch('data/nav_summary.json');
      const usd = (n) => `$${Number(n ?? 0).toLocaleString()}`;

      const cash = sum?.latest?.cash ?? null;
      const nav  = sum?.latest?.nav ?? null;
      const date = sum?.latest?.date ?? '';

      const cashEl = $('#cashAmt');
      const pctEl  = $('#cashPct');
      const dateEl = $('#cashDate');

      if (cashEl) cashEl.textContent = cash != null ? usd(cash) : '—';
      if (pctEl)  pctEl.textContent  = (cash != null && nav) ? `${((cash / nav) * 100).toFixed(1)}%` : '—';
      if (dateEl) dateEl.textContent = date || '—';
    } catch (e) {
      console.warn('cash card: could not read nav_summary.json', e);
    }
  }
