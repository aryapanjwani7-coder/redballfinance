...
TAB_GIDS_JSON = os.getenv("TAB_GIDS_JSON", "").strip()  # optional secret
ROOT = Path(__file__).resolve().parents[2]
DATA = ROOT / "data"; QUOTES = DATA / "quotes"; QUOTES.mkdir(parents=True, exist_ok=True)
stocks_json = DATA / "stocks.json"; tx_json = DATA / "transactions.json"; tab_gids_path = DATA / "tab_gids.json"

def die(msg, code=1): print(f"ERROR: {msg}", file=sys.stderr); sys.exit(code)
def default_tab(symbol: str) -> str: return (symbol or "").upper().replace(".", "_")
...
def local_cur(symbol: str) -> str: return "INR" if symbol.upper().endswith((".NS",".BO")) else "USD"

def load_stocks():
    if not stocks_json.exists(): die("data/stocks.json not found")
    try: stocks = json.loads(stocks_json.read_text(encoding="utf-8"))
    except Exception as e: die(f"failed to parse data/stocks.json: {e}")
    if not isinstance(stocks, list) or not stocks: die("data/stocks.json must be a non-empty JSON array")
    return stocks

def synth_tx(stocks):
    rows=[]
    for s in stocks:
        qty=float(s.get("qty") or 0); bd=s.get("buy_date")
        if qty>0 and bd:
...
def align(index: pd.Series, s: pd.Series)->pd.Series: return s.reindex(index).ffill()

def main():
    if not SHEET_ID: die("SHEET_ID missing")
    ...
    if not symbols: die("no usable symbols in data/stocks.json")
    ...
    for sym in symbols:
        ...
        if df is None:
            tab = sym if sym=="USDINR" else sym.upper().replace(".","_")
            try: df = fetch_tab_csv(SHEET_ID, tab)
            except Exception as e: print(f"[warn] tab fetch failed for {sym} (tab {tab}): {e}"); df=pd.DataFrame(columns=["date","close"])
        (QUOTES / f"{sym}.json").write_text(df.to_json(orient="records", force_ascii=False), encoding="utf-8")
        price_map[sym]=df; all_dates |= set(df["date"].unique())
        print(f"[ok] wrote data/quotes/{sym}.json rows={len(df)}")

    if not all_dates: die("no quote data fetched (sharing/publish or mapping issue)")
    ...
    fx_df = None
    if fx_gid:
        try: fx_df = fetch_gid_csv(SHEET_ID, fx_gid)
    ...
    if fx_df is not None and len(fx_df):
        fx = fx_df.set_index("date")["close"]; have_fx=True
    ...
    if tx_json.exists():
        try:
            tx = pd.read_json(tx_json); tx["date"]=pd.to_datetime(tx["date"]).dt.date.astype("string")
            tx = tx[tx["symbol"].isin(symbols)]
            print(f"[info] transactions loaded: {len(tx)} rows")
        except Exception as e: die(f"failed to read data/transactions.json: {e}")
    else:
        tx = synth_tx(stocks); print(f"[info] synthesized transactions: {len(tx)} rows")
    ...
    nav_df=pd.DataFrame({
        ...
        "invested_usd":invested_usd.round(4),"nav_index":nav_idx,"pnl_abs_usd":pnl_abs,"pnl_pct":pnl_pct
    })
    (DATA/"nav.json").write_text(nav_df.to_json(orient="records", force_ascii=False), encoding="utf-8")
    summary={"base_currency":"USD","starting_cash":STARTING_CASH,"inception_date":inception if inception else None,
             "latest":{"date":all_dates.iloc[-1],"nav":float(nav_usd.iloc[-1]),"cash":float(cash_usd.iloc[-1]),
                       "holdings":float(values_usd.iloc[-1]),"invested":float(invested_usd.iloc[-1]),
